...
import advisorsData from '@/data/mock/advisors.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetAdvisors') || query.includes('nodeAdvisors')) {
      return loadMockData('advisors.json')
    }
