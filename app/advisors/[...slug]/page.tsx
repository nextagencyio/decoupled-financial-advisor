...
import { GET_ADVISOR_BY_PATH } from '@/lib/queries'
import { DrupalAdvisor } from '@/lib/types'
...
interface AdvisorByPathData {
  route: {
    entity: DrupalAdvisor
  } | null
}
...
async function getAdvisor(path: string): Promise<DrupalAdvisor | null> {
  try {
    ...
    const { data } = await apolloClient.query<AdvisorByPathData>({
      query: GET_ADVISOR_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/advisors/${slug.join('/')}`
  const item = await getAdvisor(path)

  if (!item) {
    return { title: 'Advisor Not Found | Financial Advisor' }
  }

  return {
    title: `${item.title} | Financial Advisor`,
